...
public class DatabaseManagerEvents {
    public static final String DB_NAME = "Events";
    public static final String DB_TABLE = "Events";
    public static final int DB_VERSION = 1;
    private static final String CREATE_TABLE = "CREATE TABLE " + DB_TABLE + " (eventID INTEGER PRIMARY KEY, eventName TEXT, date DATE, time TIME, location TEXT);";
    private SQLHelper helper;
    private SQLiteDatabase db;
    private Context context;

    public DatabaseManagerEvents(Context c) {
        this.context = c;
        helper = new SQLHelper(c);
        this.db = helper.getWritableDatabase();
    }

    public DatabaseManagerEvents openReadable() throws android.database.SQLException {
    ...
    public boolean addRow( String en, String date, String time, String loc) {
        synchronized(this.db) {

            ContentValues newProduct = new ContentValues();
            //newProduct.put("id ", id);
            newProduct.put("eventName", en);
            newProduct.put("date", date);
            newProduct.put("time", time);
            newProduct.put("location", loc);
            try {
    ...
    public ArrayList<String> retrieveRows() {
        ...
        String[] columns = new String[] {"eventID", "eventName", "date", "time", "location"};
        Cursor cursor = db.query(DB_TABLE, columns, null, null, null, null, null);
        cursor.moveToFirst();
        while (cursor.isAfterLast() == false) {
            productRows.add(Integer.toString(cursor.getInt(0)) + ", " + cursor.getString(1) + ", " + cursor.getString(2) + ", " + cursor.getString(3) + ", " + cursor.getString(4)   );
            cursor.moveToNext();
        }
